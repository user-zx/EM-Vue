		...
		<div class="art-content">
			...
			<div class="sellClue_list_div" v-for="(artItem,index) in artList.artContent" v-if="artItem.isShow">
                <div>
                    ...
                    <div class="sellClue_list_div_div"> <span><i>发布者:</i>{{artItem.salesLeads.author}}</span><span v-if="artItem.salesLeads.matchingResult=='匹配成功'"><i >匹配时间:</i>{{artItem.salesLeads.matchingDate}}</span><span v-else><i >提交时间:</i>{{artItem.salesLeads.publishDate}}</span><span><i>线索来源:</i>{{artItem.salesLeads.source}}</span></div>
                    ...
                    <button class="btn btn-search" v-if="!artItem.checkStatus&&artItem.salesLeads.matchingResult!='暂未匹配'" @click="getLinkStatus(index,artItem.salesLeads.id)">联系人信息</button>
                </div>  
				<menu class="clearfix">   
